...
long int mat_mult_MAX(long int* matrix1, long int* matrix2, int m, int n, int p, int threads) {
  ...
  #pragma omp parallel for private(i, j, k) reduction(max:maximum2) num_threads(threads)
  for (i = 0; i < m; i++) {
    for (j = 0; j < p; j++) {
      int sum = 0;
      for (k = 0; k < n; k++) {
...
int main(int argc, char* argv[])
{
    ...
    long int maximumSecond = mat_mult_MAX(matrix1, matrix2, m, n, p, thread_count);

    // Record the finish time        
    double end = omp_get_wtime();
    
    // Write the result maximum to the file
    fprintf(outputFile, "%ld", maximumSecond);
